...
export default function CreationCardMinimal({ creation }) {
  ...
  const { key, address, uri, timestamp, prompt, status } = creation

  const [modalOpen, setModalOpen] = useState(false)
  const handleModalOpen = () => {
    router.push(`/creation/${key}`, `/creation/${key}`, { shallow: true })
    setModalOpen(true)
  }
  ...
  return (
    <CardStyles>
      <article id="creation-card" onClick={handleModalOpen}>
